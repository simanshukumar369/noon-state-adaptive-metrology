...
├── LICENSE
└── figures/
    ├── fig1_circuit.png         # Circuit schematic — preview (used in this README)
    ├── main/                    # All 13 paper figures (PDF)
    │   ├── fig1_circuit.pdf
    │   ├── fig8_fim_bars.pdf
    │   ├── fig2_fringe_gallery.pdf
    │   ├── fig4_pareto.pdf
    │   ├── figW1_gallery.pdf
    │   ├── figW2_negativity.pdf
    │   ├── figW3_portrait.pdf
    │   ├── figW4_evolution.pdf
    │   ├── fig3_scaling_summary.pdf
    │   ├── fig5_convergence.pdf
    │   ├── fig6_param_drift.pdf
    │   ├── fig7_photon_dist.pdf
    │   └── fig_qfi.pdf
    └── supplementary/           # Additional figures generated by code (PNG)
        └── ...
```
...
> All 13 paper figures (PDF) are in [`figures/main/`](figures/main/) and can be browsed directly on GitHub.  
> Additional exploratory figures generated by the code but not included in the paper are in [`figures/supplementary/`](figures/supplementary/).
...
Generated figures are written to `figures/main/` and `figures/supplementary/`. Pickled results are written to the working directory.
...
|------|---------|:------------:|
| `figures/main/fig1_circuit.pdf` | Circuit schematic (paper uses LaTeX/quantikz version) | Fig. 1 |
| `figures/main/fig8_fim_bars.pdf` | Raw CFI bar chart, Afek vs. optimised | Fig. 2 |
| `figures/main/fig2_fringe_gallery.pdf` | Coincidence fringe gallery for all N and channels | Fig. 3 |
| `figures/main/fig4_pareto.pdf` | Pareto trade-off: fringe quality vs. post-selection rate | Fig. 4 |
| `figures/main/figW1_gallery.pdf` | Wigner function gallery (all N) | Fig. 5 |
| `figures/main/figW2_negativity.pdf` | Wigner negativity comparison, Afek vs. optimised | Fig. 6 |
| `figures/main/figW3_portrait.pdf` | Phase-space portrait of quantum states (N=2) | Fig. 7 |
| `figures/main/figW4_evolution.pdf` | State evolution through circuit stages | Fig. 8 |
| `figures/main/fig3_scaling_summary.pdf` | CFI and rate scaling with N | Fig. 9 |
| `figures/main/fig5_convergence.pdf` | Adam training convergence curves | Fig. 10 |
| `figures/main/fig6_param_drift.pdf` | Parameter drift from Afek initialisation | Fig. 11 |
| `figures/main/fig7_photon_dist.pdf` | Marginal photon-number distributions | Fig. 12 |
| `figures/main/fig_qfi.pdf` | QFI and measurement-efficiency analysis | Fig. 13 |
